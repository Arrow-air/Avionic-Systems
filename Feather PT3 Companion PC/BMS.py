    ...
    def packetStruct(self):
        ...
        for x in range(10,15):
           if self.packet['unit_id'] == x:
               y = x - 9
               self.dataDictionary['BAT{y}_temp_C'] = self.packet['CAN_PACKET_BMS_TEMP0']
               self.dataDictionary['BAT{y}_soc_PCT'] = self.packet['SOC']
               self.dataDictionary['ESC{y}_V'] = self.packet['packVoltage']
               self.dataDictionary['ESC{y}_CUR_AMP'] = self.packet['packCurrent1']
